...
public class TiamatStrongHoldInstance extends GeneralInstanceHandler {
	...
	public void onDie(Npc npc) {
		...
		switch (npc.getNpcId()) {
			...
			case 219352: // Invincible Shabokan
				sendMsg(1401614);
				spawn(701501, 1075.4409f, 1078.5071f, 787.685f, (byte) 16); // balaur
																			// medal
																			// box
			...
			case 219357: // Adjutant Anuhart
				sendMsg(1401614);
				spawn(701501, 1077.1716f, 1058.1995f, 787.685f, (byte) 61); // balaur
																			// medal
																			// box
			...
			case 219358: // Brigade General Tahabata
				sendMsg(1401614);
				spawn(701541, 677.35785f, 1069.5361f, 499.86716f, (byte) 0); // tahabata's
																				// treasure
																				// chest
				spawn(730622, 652.4821f, 1069.0302f, 498.7787f, (byte) 0, 82); // central
																				// passage
																				// teleport
				spawn(283180, 679.88f, 1068.88f, 504.2f, (byte) 119); // tiamat's
																		// eye
				isDeadBosses();
				break;
			case 219353: // Brigade General Chantra
				sendMsg(1401614);
				spawn(701501, 1071.5909f, 1040.6797f, 787.685f, (byte) 23); // balaur
																			// medal
																			// box
			...
			case 219354: // Brigade General Terath
				sendMsg(1401614);
				spawn(283178, 1030.03f, 301.83f, 411f, (byte) 26); // tiamat's
																	// eye
				spawn(701501, 1086.274f, 1098.3997f, 787.685f, (byte) 90); // balaur
																			// medal
																			// box
				spawn(730622, 1029.792f, 267.0502f, 409.7982f, (byte) 0, 83); // central
																				// passage
																				// teleport
				isDeadBosses();
				break;
			case 219355: // Traitor Kumbanda
				sendMsg(1401614);
				spawn(701501, 1063.5973f, 1092.7402f, 787.685f, (byte) 107); // balaur
																				// medal
																				// box
			...
			case 219356: // Brigade General Laksyaka
				sendMsg(1401614);
				spawn(701501, 1099.8691f, 1047.1895f, 787.685f, (byte) 64); // balaur
																			// medal
																			// box
				spawn(730622, 644.4221f, 1319.6221f, 488.7422f, (byte) 0, 15); // central
																				// passage
																				// teleport
				spawn(701527, 1073.948f, 1068.8732f, 787.685f, (byte) 61); // stronghold
																			// treasure
																			// chest
				spawn(800438, 665.63409f, 1319.7051f, 487.9f, (byte) 61); // murugan
																			// the
																			// loyal
				spawn(283179, 629.1f, 1319.5f, 501.2f, (byte) 0); // tiamat's
																	// eye
				isDeadBosses();
				break;
		}
	...
	private void spawnKahrun() {
		ThreadPoolManager.getInstance().schedule(new Runnable() {
			@Override
			public void run() {
				moveToForward((Npc) spawn(800463, 1201.272f, 1074.5463f, 491f, (byte) 61), 1039.5f, 1075.9f, 497.3f, false); // elite
																																// reian
																																// fighter
				...
				moveToForward((Npc) spawn(800380, 1190.323f, 1068.1558f, 491.03488f, (byte) 61), 1108, 1066, 497.3f, false); // elite
																																// reian
																																// sniper
				moveToForward((Npc) spawn(800374, 1188.4259f, 1066.4757f, 491.55029f, (byte) 61), 1094, 1064, 497.4f, true); // elite
																																// reian
																																// assaulter
				moveToForward((Npc) spawn(800374, 1188.2158f, 1074.2047f, 491.55029f, (byte) 61), 1092.5f, 1074.6f, 497.4f, true);
				moveToForward((Npc) spawn(800376, 1190.3859f, 1071.6548f, 491.03488f, (byte) 61), 1109, 1073, 497.2f, false); // elite
																																// reian
																																// assaulter
	...
	private boolean isDeadBosses() {
		...
		if (isDead(boss) && isDead(boss1) && isDead(boss2) && isDead(boss3) && isDead(boss4) && isDead(boss5) && isDead(boss6)) {
			spawn(800464, 1119.7076f, 1071.1401f, 496.8615f, (byte) 119); // reian
																			// sorcerer
			spawn(800465, 1119.7421f, 1068.4998f, 496.8616f, (byte) 3); // reian
																		// sorcerer
			spawn(730629, 1121.3807f, 1069.8124f, 500.3319f, (byte) 0, 555); // tiamat
																				// stronghold
																				// exit
			return true;
		}
	...
	public void onEnterZone(Player player, ZoneInstance zone) {
		if (zone.getAreaTemplate().getZoneName() == ZoneName.get("LAKSYAKA_LEGION_HQ_300510000")) {
			if (!startSuramaEvent) {
				...
				spawn(800433, 725.93f, 1319.9f, 490.7f, (byte) 61); // surama
																	// the
																	// traitor
			}
		}
