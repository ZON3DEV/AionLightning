...
public class ShugoImperialTombInstance extends GeneralInstanceHandler {
		...
		switch (list) {
			...
			case START_STAGE_1_PHASE_2:
				...
				ThreadPoolManager.getInstance().schedule(new Runnable() {
					@Override
					public void run() {
						sendMsg(1401665); // Grave Robbers will attack again in
											// 5 seconds!
						ThreadPoolManager.getInstance().schedule(new Runnable() {
							@Override
							public void run() {
			...
			case START_STAGE_3_PHASE_2:
				...
				ThreadPoolManager.getInstance().schedule(new Runnable() {
					@Override
					public void run() {
						sendMsg(1401671); // Grave Robbers will attack in again
											// in 5 seconds!
						ThreadPoolManager.getInstance().schedule(new Runnable() {
							@Override
							public void run() {
	...
	private void startBonusStage1() {
		...
		ThreadPoolManager.getInstance().schedule(new Runnable() {
			@Override
			public void run() {
				sendMsg(1401607); // Grave Robbers will attack again in 5
									// seconds!
				ThreadPoolManager.getInstance().schedule(new Runnable() {
					@Override
					public void run() {
