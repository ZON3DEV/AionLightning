...
public class InGameShopEn {
	...
	public void acceptRequest(Player player, int itemObjId) {
		...
		if (LoggingConfig.LOG_INGAMESHOP) {
			log.info("[INGAMESHOP] > Account name: " + player.getAcountName() + ", PlayerName: " + player.getName() + " is watching item:" + item.getItemId()
	...
	private boolean sendShopItemGift(Player player, IGItem shopItem, String reciverName) {
		...
		try {
			SystemMailService.getInstance().sendMail("$$CASH_ITEM_MAIL", reciverName, mailSubject, mailMessage, shopItem.getItemId(), shopItem.getItemCount(),
