...
public class BeshmundirsWalkAI2 extends ActionItemNpcAI2 {
	...
	protected void handleUseItemFinish(Player player) {
		PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(getObjectId(), 10)); // Initial
																						// dialog
	}
		...
		switch (dialogId) {
			...
			case 65:// I'm ready to enter
				...
				if (player.getPlayerGroup2().isLeader(player)) {
					PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(getObjectId(), 4762)); // Path
																										// selection
				} else {
