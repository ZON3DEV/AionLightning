		...
		if (target instanceof Player) {
			...
			PacketSendUtility.sendMessage(admin,
					"[Info about " + player.getName() + "]" + "\nPlayer Id: " + player.getObjectId() + "\nMap ID: " + player.getWorldId() + "\nX: "
							+ player.getCommonData().getPosition().getX() + " / Y: " + player.getCommonData().getPosition().getY() + " / Z: "
							+ player.getCommonData().getPosition().getZ() + " / Heading: " + player.getCommonData().getPosition().getHeading() + "\n Town ID: "
			...
			for (int i = 0; i < 2; i++) {
				...
				if (faction != null) {
					PacketSendUtility.sendMessage(admin, player.getName()
							...
							+ "\nCurrent quest state: "
							+ faction.getState().name()
							+ (faction.getState().equals(ENpcFactionQuestState.COMPLETE) ? ("\nNext after: "
									+ ((faction.getTime() - System.currentTimeMillis() / 1000) / 3600f) + " h.") : ""));
				}
			}
			...
			PacketSendUtility.sendMessage(admin, "[Info about target]" + "\nName: " + npc.getName() + "\nId: " + npc.getNpcId() + " / ObjectId: "
					...
					+ admin.getTarget().getHeading() + " / Angle: " + PositionUtil.getAngleToTarget(admin, admin.getTarget()) + " \n Town ID:"
			...
			PacketSendUtility.sendMessage(admin, "[Sense range]" + "\nRadius: " + npc.getObjectTemplate().getAggroRange() + "\nSide: "
					...
					+ (npc.getObjectTemplate().getAggroRange() + PositionUtil.getDirectionalBound(npc, admin, true)) + "\nCollision: "
					+ (npc.getObjectTemplate().getAggroRange() - npc.getCollision()));
			int asmoDmg = 0;
			int elyDmg = 0;
