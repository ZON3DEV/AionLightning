...
public class Skill {
	...
	public enum SkillMethod {
		...
		CAST, ITEM, PASSIVE, PROVOKED, CHARGE
	}
	...
	protected void calculateSkillDuration() {
		...
		// No cast speed cap for skill Summoning Alacrity I(skillId: 1778) and
		// Nimble Fingers I(skillId: 2386)
	...
	private boolean checkAnimationTime() {
		if (!(effector instanceof Player) || skillMethod != SkillMethod.CAST)// TODO
																				// item
																				// skills?
		{
			return true;
		...
		if (getSkillTemplate().getAmmoSpeed() != 0) {
			ammoTime = Math.round(distance / getSkillTemplate().getAmmoSpeed() * 1000);// checked
																						// with
																						// client
		}
		clientTime -= ammoTime;
		...
		if (motion.getInstantSkill() && hitTime == 0) {
			...
			if (clientTime < finalTime) {
				...
				log.warn("Possible modified client_skills:" + this.getSkillId() + " (clientTime<finalTime:" + clientTime + "/" + finalTime + ") player Name: "
						+ player.getName());
			}
	 ...
	 *         comes from old 1.5.0.5 patch notes and still applies on 2.5
	 *         (confirmed) TODO: maybe another implementation? At the moment
	 *         this doesnt seem to be handled on client infos, so it's hard
	 *         coded
	 */
	...
	public boolean shouldAffectTarget(VisibleObject object) {
		// If creature is at least 2 meters above the terrain, ground skill
		// cannot be applied
