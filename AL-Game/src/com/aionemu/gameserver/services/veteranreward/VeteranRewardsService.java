...
public class VeteranRewardsService {
	...
	private void StartVeteranReward() {
		...
		for (final VeteranRewards veteran_reward : veteran_rewards) {
			VerifyVeteranReward(veteran_reward.getId(), veteran_reward.getPlayer(), veteran_reward.getType(), veteran_reward.getItem(),
					veteran_reward.getCount(), veteran_reward.getKinah(), veteran_reward.getSender(), veteran_reward.getTitle(), veteran_reward.getMessage());
		}
	}
	...
	private void SendVeteranRewardMail(String sender, String recipientName, String title, String message, int attachedItemObjId, int attachedItemCount,
		...
		Letter newLetter = new Letter(IDFactory.getInstance().nextId(), recipientCommonData.getPlayerObjId(), attachedItem, finalAttachedKinahCount, title,
				message, finalSender, time, true, type);
