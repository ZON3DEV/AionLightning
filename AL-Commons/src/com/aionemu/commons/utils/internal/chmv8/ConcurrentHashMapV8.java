 ...
 * specification as {@link java.util.Hashtable}, and includes versions of methods corresponding to each method of {@code Hashtable}.
 * However, even though all operations are thread-safe, retrieval operations do <em>not</em> entail locking, and there is <em>not</em> any
 * support for locking the entire table in a way that prevents all access. This class is fully inter-operable with {@code Hashtable} in
 * programs that rely on its thread safety but not on its synchronization details.
 *
 * <p>Retrieval operations (including {@code get}) generally do not block, so may overlap with update operations (including {@code put} and
 * {@code remove}). Retrievals reflect the results of the most recently <em>completed</em> update operations holding upon their onset. (More
 * formally, an update operation for a given key bears a
 * <em>happens-before</em> relation with any (non-null) retrieval for that key reporting the updated value.) For aggregate operations such
 * as {@code putAll} and {@code clear}, concurrent retrievals may reflect insertion or removal of only some entries. Similarly, Iterators
 * and Enumerations return elements reflecting the state of the hash table at some point at or since the creation of the
 * iterator/enumeration. They do <em>not</em> throw {@link
 * ConcurrentModificationException}. However, iterators are designed to be used by only one thread at a time. Bear in mind that the results
 * of aggregate status methods including {@code size}, {@code
 * isEmpty}, and {@code containsValue} are typically useful only when a map is not undergoing concurrent updates in other threads. Otherwise
 * the results of these methods reflect transient states that may be adequate for monitoring or estimation purposes, but not for program
 * control.
 *
 * <p>The table is dynamically expanded when there are too many collisions (i.e., keys that have distinct hash codes but fall into the same
 * slot modulo the table size), with the expected average effect of maintaining roughly two bins per mapping (corresponding to a 0.75 load
 * factor threshold for resizing). There may be much variance around this average as mappings are added and removed, but overall, this
 * maintains a commonly accepted time/space tradeoff for hash tables. However, resizing this or any other kind of hash table may be a
 * relatively slow operation. When possible, it is a good idea to provide a size estimate as an optional {@code
 * initialCapacity} constructor argument. An additional optional {@code loadFactor} constructor argument provides a further means of
 * customizing initial table capacity by specifying the table density to be used in calculating the amount of space to allocate for the
 * given number of elements. Also, for compatibility with previous versions of this class, constructors may optionally specify an expected
 * {@code concurrencyLevel} as an additional hint for internal sizing. Note that using many keys with exactly the same {@code hashCode()} is
 * a sure way to slow down performance of any hash table.
 *
 * <p>A {@link Set} projection of a ConcurrentHashMapV8 may be created (using {@link #newKeySet()} or {@link #newKeySet(int)}), or viewed
 * (using {@link #keySet(Object)} when only keys are of interest, and the mapped values are (perhaps transiently) not used or all take the
 * same mapping value.
 *
 * <p>A ConcurrentHashMapV8 can be used as scalable frequency map (a form of histogram or multiset) by using {@link LongAdder} values and
 * initializing via {@link #computeIfAbsent}. For example, to add a count to a {@code ConcurrentHashMapV8<String,LongAdder> freqs}, you can
 * use {@code freqs.computeIfAbsent(k -> new
 * LongAdder()).increment();}
 *
 * <p>This class and its views and iterators implement all of the
 * <em>optional</em> methods of the {@link Map} and {@link Iterator} interfaces.
 *
 * <p>Like {@link Hashtable} but unlike {@link HashMap}, this class does <em>not</em> allow {@code null} to be used as a key or value.
 *
 * <p>ConcurrentHashMapV8s support sequential and parallel operations bulk operations. (Parallel forms use the {@link
 * ForkJoinPool#commonPool()}). Tasks that may be used in other contexts are available in class {@link ForkJoinTasks}. These operations are
 * designed to be safely, and often sensibly, applied even with maps that are being concurrently updated by other threads; for example, when
 * computing a snapshot summary of the values in a shared registry. There are three kinds of operation, each with four forms, accepting
 * functions with Keys, Values, Entries, and (Key, Value) arguments and/or return values. Because the elements of a ConcurrentHashMapV8 are
 * not ordered in any particular way, and may be processed in different orders in different parallel executions, the correctness of supplied
 * functions should not depend on any ordering, or on any other objects or values that may transiently change while computation is in
 * progress; and except for forEach actions, should ideally be side-effect-free.
 *
 * <ul>
 * <li> forEach: Perform a given action on each element. A variant form applies a given transformation on each element before performing the
 * action.</li>
 *
 * <li> search: Return the first available non-null result of applying a given function on each element; skipping further search when a
 * result is found.</li>
 *
 * <li> reduce: Accumulate each element. The supplied reduction function cannot rely on ordering (more formally, it should be both
 * associative and commutative). There are five variants:
 *
 * <ul>
 *
 * <li> Plain reductions. (There is not a form of this method for (key, value) function arguments since there is no corresponding return
 * type.)</li>
 *
 ...
 * </ul>
 *
 * <p>The concurrency properties of bulk operations follow from those of ConcurrentHashMapV8: Any non-null result returned from
 * {@code get(key)} and related access methods bears a happens-before relation with the associated insertion or update. The result of any
 * bulk operation reflects the composition of these per-element relations (but is not necessarily atomic with respect to the map as a whole
 * unless it is somehow known to be quiescent). Conversely, because keys and values in the map are never null, null serves as a reliable
 * atomic indicator of the current lack of any result. To maintain this property, null serves as an implicit basis for all non-scalar
 * reduction operations. For the double, long, and int versions, the basis should be one that, when combined with any other value, returns
 * that other value (more formally, it should be the identity element for the reduction). Most common reductions have these properties; for
 * example, computing a sum with basis 0 or a minimum with basis MAX_VALUE.
 *
 * <p>Search and transformation functions provided as arguments should similarly return null to indicate the lack of any result (in which
 * case it is not used). In the case of mapped reductions, this also enables transformations to serve as filters, returning null (or, in the
 * case of primitive specializations, the identity basis) if the element should not be combined. You can create compound transformations and
 * filterings by composing them yourself under this "null means there is nothing there now" rule before using them in search or reduce
 * operations.
 *
 * <p>Methods accepting and/or returning Entry arguments maintain key-value associations. They may be useful for example when finding the
 * key for the greatest value. Note that "plain" Entry arguments can be supplied using {@code new
 * AbstractMap.SimpleEntry(k,v)}.
 *
 * <p>Bulk operations may complete abruptly, throwing an exception encountered in the application of a supplied function. Bear in mind when
 * handling such exceptions that other concurrently executing functions could also have thrown exceptions, or would have done so if the
 * first exception had not occurred.
 *
 ...
 * small maps may execute more slowly than sequential forms if the underlying work to parallelize the computation is more expensive than the
 * computation itself. Similarly, parallelization may not lead to much actual parallelism if all processors are busy performing unrelated
 * tasks.
 *
 * <p>All arguments to all task methods must be non-null.
 *
 * <p><em>jsr166e note: During transition, this class uses nested functional interfaces with different names but the same forms as those
 * expected for JDK8.</em>
 *
...
public class ConcurrentHashMapV8<K, V>
	...
	/**
	 * A partitionable iterator. A Spliterator can be traversed directly, but can also be partitioned (before traversal) by creating another
	 * Spliterator that covers a non-overlapping portion of the elements, and so may be amenable to parallel execution.
	 *
	 * <p>This interface exports a subset of expected JDK8 functionality.
	 *
	 * <p>Sample usage: Here is one (of the several) ways to compute the sum of the values held in a map using the ForkJoin framework. As
	 * illustrated here, Spliterators are well suited to designs in which a task repeatedly splits off half its work into forked subtasks
	 * until small enough to process directly, and then joins these subtasks. Variants of this style can also be used in completion-based
	 * designs.
	 *
	 * <pre>
	...
	public static interface Spliterator<T> extends Iterator<T> {
		...
		/**
		 * Returns a Spliterator covering approximately half of the elements, guaranteed not to overlap with those subsequently returned by this
		 * Spliterator. After invoking this method, the current Spliterator will <em>not</em> produce any of the elements of the returned
		 * Spliterator, but the two Spliterators together will produce all of the elements that would have been produced by this Spliterator had
		 * this method not been called. The exact number of elements produced by the returned Spliterator is not guaranteed, and may be zero
		 * (i.e., with {@code hasNext()} reporting {@code
	 ...
	 * power of two table sizes, and is further required because the top two bits of 32bit hash fields are used for control purposes.
	 */
	...
	/**
	 * The load factor for this table. Overrides of this value in constructors affect only the initial table capacity. The actual floating
	 * point value isn't normally used -- it is simpler to use expressions such as {@code n - (n >>> 2)} for the associated resizing
	 * threshold.
	 */
	private static final float LOAD_FACTOR = 0.75f;
	/**
	 * The bin count threshold for using a tree rather than list for a bin. The value reflects the approximate break-even point for using
	 * tree-based operations.
	 */
	private static final int TREE_THRESHOLD = 8;
	/**
	 * Minimum number of rebinnings per transfer step. Ranges are subdivided to allow multiple resizer threads. This value serves as a lower
	 * bound to avoid resizers encountering excessive memory contention. The value should be at least DEFAULT_CAPACITY.
	 */
	...
	/**
	 * Holder for the thread-local hash code determining which CounterCell to use. The code is initialized via the counterHashCodeGenerator,
	 * but may be moved upon collisions.
	 */
	...
	/**
	 * Table initialization and resizing control. When negative, the table is being initialized or resized: -1 for initialization, else -(1 +
	 * the number of active resizing threads). Otherwise, when table is null, holds the initial table size to use upon creation, or 0 for
	 * default. After initialization, holds the next element count value upon which to resize the table.
	 */
	...
	/* ---------------- Nodes -------------- */
	/**
	 * Key-value entry. Note that this is never exported out as a user-visible Map.Entry (see MapEntry below). Nodes with a hash field of
	 * MOVED are special, and do not contain user keys or values. Otherwise, keys are never null, and null val fields indicate that a node is
	 * in the process of being deleted or created. For purposes of read-only access, a key may be read before a val, but can only be used
	 ...
	 *
	 * TreeBins use a special form of comparison for search and related operations (which is the main reason we cannot use existing
	 * collections such as TreeMaps). TreeBins contain Comparable elements, but may contain others, as well as elements that are Comparable
	 * but not necessarily Comparable<T>
	 * for the same T, so we cannot invoke compareTo among them. To handle this, the tree is ordered primarily by hash value, then by
	 * getClass().getName() order, and then by Comparator order among elements of the same class. On lookup at a node, if elements are not
	 * comparable or compare as 0, both left and right children may need to be searched in the case of tied hash values. (This corresponds to
	 * the full list search that would be necessary if all elements were non-Comparable and had tied hashes.) The red-black balancing code is
	 * updated from pre-jdk-collections (http://gee.cs.oswego.edu/dl/classes/collections/RBCell.java) based in turn on Cormen, Leiserson, and
	 * Rivest "Introduction to Algorithms" (CLR).
	 *
	 * TreeBins also maintain a separate locking discipline than regular bins. Because they are forwarded via special MOVED nodes at bin heads
	 * (which can never change once established), we cannot use those nodes as locks. Instead, TreeBin extends AbstractQueuedSynchronizer to
	 * support a simple form of read-write lock. For update operations and table validation, the exclusive form of lock behaves in the same
	 * way as bin-head locks. However, lookups use shared read-lock mechanics to allow multiple readers in the absence of writers.
	 * Additionally, these lookups do not ever block: While the lock is not available, they proceed along the slow traversal path (via
	 * next-pointers) until the lock becomes available or the list is exhausted, whichever comes first. (These cases are not fast, but
	 * maximize aggregate expected throughput.) The AQS mechanics for doing this are straightforward. The lock state is held as AQS
	 * getState(). Read counts are negative; the write count (1) is positive. There are no signalling preferences among readers and writers.
	 * Since we don't need to export full Lock API, we just override the minimal AQS methods and use them directly.
	 */
	static final class TreeBin<V> extends AbstractQueuedSynchronizer {
		...
		/**
		 * Wrapper for getTreeNode used by CHM.get. Tries to obtain read-lock to call getTreeNode, but during failure to get lock, searches
		 * along next links.
		 */
		...
		/**
		 * Removes the given node, that must be present before this call. This is messier than typical red-black deletion code because we cannot
		 * swap the contents of an interior node with a leaf successor that is pinned by "next" pointers that are accessible independently of
		 * lock. So instead we swap the tree linkages.
		 */
	...
	/**
	 * Spreads higher bits to lower, and also forces top bit to 0. Because the table uses power-of-two masking, sets of hashes that vary only
	 * in bits above the current mask will always collide. (Among known examples are sets of Float keys holding consecutive whole numbers in
	 * small tables.) To counter this, we apply a transform that spreads the impact of higher bits downward. There is a tradeoff between
	 * speed, utility, and quality of bit-spreading. Because many common sets of hashes are already reasonably distributed across bits (so
	 * don't benefit from spreading), and because we use trees to handle large sets of collisions in bins, we don't need excessively high
	 * quality.
	 */
	...
	/**
	 * Implementation for the four public remove/replace methods: Replaces node value with v, conditional upon match of cv if non-null. If
	...
	/**
	 * Adds to count, and if table is too small and not already resizing, initiates transfer. If already resizing, helps perform transfer if
	 * work is available. Rechecks occupancy after a transfer to see if another resize is already needed because resizings are lagging
	 * additions.
	 *
	 ...
	 *
	 * At each step, the iterator snapshots the key ("nextKey") and value ("nextVal") of a valid node (i.e., one that, at point of snapshot,
	 * has a non-null user value). Because val fields can change (including to null, indicating deletion), field nextVal might not be accurate
	 * at point of use, but still maintains the weak consistency property of holding a value that was once valid. To support iterator.remove,
	 * the nextKey field is not updated (nulled out) when the iterator cannot advance.
	 *
	 * Internal traversals directly access these fields, as in: {@code while (it.advance() != null) { process(it.nextKey); }}
	 *
	 * Exported iterators must track whether the iterator has advanced (in hasNext vs next) (by setting/checking/nulling field nextVal), and
	 * then extract key, value, or key-value pairs as return values of next().
	 *
	 * The iterator visits once each still-valid node that was reachable upon iterator construction. It might miss some that were added to a
	 * bin after the bin was visited, which is OK wrt consistency guarantees. Maintaining this property in the face of possible ongoing
	 * resizes requires a fair amount of bookkeeping state that is difficult to optimize away amidst volatile accesses. Even so, traversal
	 * maintains reasonable throughput.
	 *
	 * Normally, iteration proceeds bin-by-bin traversing lists. However, if the table has been resized, then all future steps must traverse
	 * both the bin at the current index as well as at (index + baseSize); and so on for further resizings. To paranoically cope with
	 * potential sharing by users of iterators across threads, iteration terminates if a bounds checks fails for a table read.
	 *
	 * This class extends CountedCompleter to streamline parallel iteration in bulk operations. This adds only a few fields of space overhead,
	 * which is small enough in cases where it is not needed to not worry about it. Because CountedCompleter is Serializable, but iterators
	 * need not be, we need to add warning suppressions.
	 */
	...
	static class Traverser<K, V, R>
		...
		/**
		 * Returns a batch value > 0 if this task should (and must) be split, if so, adding to pending count, and in any case updating batch
		 * value. The initial batch value is approx exp2 of the number of times (minus one) to split task by two before executing leaf action.
		 * This value is faster to compute and more convenient to use as a guide to splitting than is the depth, since it is used while dividing
		 * by two anyway.
		 */
		final int preSplit() {
			ConcurrentHashMapV8<K, V> m;
	...
	/**
	 * Creates a new, empty map with an initial table size accommodating the specified number of elements without the need to dynamically
	 * resize.
	 *
	...
	/**
	 * Creates a new, empty map with an initial table size based on the given number of elements ({@code initialCapacity}) and initial table
	 * density ({@code loadFactor}).
	 *
	 * @param initialCapacity the initial capacity. The implementation performs internal sizing to accommodate this many elements, given the
	...
	/**
	 * Creates a new, empty map with an initial table size based on the given number of elements ({@code initialCapacity}), table density
	 * ({@code loadFactor}), and number of concurrently updating threads ({@code concurrencyLevel}).
	 *
	 * @param initialCapacity the initial capacity. The implementation performs internal sizing to accommodate this many elements, given the
	 * specified load factor.
	 * @param loadFactor the load factor (table density) for establishing the initial table size
	...
	/**
	 * Returns the number of mappings. This method should be used instead of {@link #size} because a ConcurrentHashMapV8 may contain more
	 * mappings than can be represented as an int. The value returned is an estimate; the actual count may differ if there are concurrent
	 ...
	 *
	 * <p>More formally, if this map contains a mapping from a key {@code k} to a value {@code v} such that {@code key.equals(k)}, then this
	 * method returns {@code v}; otherwise it returns {@code null}. (There can be at most one such mapping.)
	 *
	 ...
	 * @return {@code true} if and only if the specified object is a key in this table, as determined by the {@code equals} method;
	...
	/**
	 * Returns {@code true} if this map maps one or more keys to the specified value. Note: This method may require a full traversal of the
	 * map, and is much slower than method {@code containsKey}.
	 *
	 ...
	 * {@link #containsValue}, and exists solely to ensure full compatibility with class {@link java.util.Hashtable}, which supported this
	 * method prior to introduction of the Java Collections framework.
	 *
	 * @param value a value to search for
	 * @return {@code true} if and only if some key maps to the {@code value} argument in this table as determined by the {@code equals}
	 ...
	 *
	 * <p>The value can be retrieved by calling the {@code get} method with a key that is equal to the original key.
	 *
	...
	/**
	 * Copies all of the mappings from the specified map to this one. These mappings replace any mappings that this map had for any of the
	...
	/**
	 * If the specified key is not already associated with a value, computes its value using the given mappingFunction and enters it into the
	 * map unless null. This is equivalent to
	 * <pre> {@code
	 ...
	 * return value;}</pre>
	 *
	 * except that the action is performed atomically. If the function returns {@code null} no mapping is recorded. If the function itself
	 * throws an (unchecked) exception, the exception is rethrown to its caller, and no mapping is recorded. Some attempted update operations
	 * on this map by other threads may be blocked while computation is in progress, so the computation should be short and simple, and must
	 * not attempt to update any other mappings of this Map. The most appropriate usage is to construct a new object serving as an initial
	 ...
	 * }</pre>
	 *
	 * except that the action is performed atomically. If the function returns {@code null}, the mapping is removed. If the function itself
	 * throws an (unchecked) exception, the exception is rethrown to its caller, and the current mapping is left unchanged. Some attempted
	 * update operations on this map by other threads may be blocked while computation is in progress, so the computation should be short and
	 * simple, and must not attempt to update any other mappings of this Map. For example, to either create or append new messages to a value
	 * mapping:
	 *
	 * @param key key with which the specified value is to be associated
	...
	/**
	 * Computes a new mapping value given a key and its current mapped value (or {@code null} if there is no current mapping). This is
	 * equivalent to
	 * <pre> {@code
	 *   value = remappingFunction.apply(key, map.get(key));
	 *   if (value != null)
	...
	/**
	 * If the specified key is not already associated with a value, associate it with the given value. Otherwise, replace the value with the
	 * results of the given remapping function. This is equivalent to:
	 * <pre> {@code
	 *   if (!map.containsKey(key))
	 *     map.put(value);
	 *   else {
	 ...
	 *       map.remove(key);
	 *   }
	 * }</pre> except that the action is performed atomically. If the function returns {@code null}, the mapping is removed. If the function
	 * itself throws an (unchecked) exception, the exception is rethrown to its caller, and the current mapping is left unchanged. Some
	 * attempted update operations on this map by other threads may be blocked while computation is in progress, so the computation should be
	 * short and simple, and must not attempt to update any other mappings of this Map.
	 */
	...
	/**
	 * Returns a {@link Set} view of the keys contained in this map. The set is backed by the map, so changes to the map are reflected in the
	 * set, and vice-versa.
	 *
	 * @return the set view
	 */
	...
	/**
	 * Returns a {@link Set} view of the keys in this map, using the given common mapped value for any additions (i.e., {@link
	 * Collection#add} and {@link Collection#addAll}). This is of course only appropriate if it is acceptable to use the same value for all
	 * additions from this view.
	 *
	...
	/**
	 * Returns a {@link Collection} view of the values contained in this map. The collection is backed by the map, so changes to the map are
	...
	/**
	 * Returns a {@link Set} view of the mappings contained in this map. The set is backed by the map, so changes to the map are reflected in
	 * the set, and vice-versa. The set supports element removal, which removes the corresponding mapping from the map, via the {@code Iterator.remove}, {@code Set.remove},
	 * {@code removeAll}, {@code retainAll}, and {@code clear} operations. It does not support the {@code add} or {@code addAll} operations.
	 *
	 * <p>The view's {@code iterator} is a "weakly consistent" iterator that will never throw {@link ConcurrentModificationException}, and
	...
	/**
	 * Returns a string representation of this map. The string representation consists of a list of key-value mappings (in no particular
	 * order) enclosed in braces ("{@code {}}"). Adjacent mappings are separated by the characters {@code ", "} (comma and space). Each
	 * key-value mapping is rendered as the key followed by an equals sign ("{@code =}") followed by the associated value.
	 *
	...
	/**
	 * Compares the specified object with this map for equality. Returns {@code true} if the given object is a map with the same mappings as
	 * this map. This operation may return misleading results if either map is concurrently modified during execution of this method.
	 *
	...
	static final class MapEntry<K, V> implements Map.Entry<K, V> {
		...
		/**
		 * Sets our entry's value and writes through to the map. The value to return is somewhat arbitrary here. Since we do not necessarily
		 * track asynchronous changes, the most recent "previous" value could be different from what we return (or could even have been removed
	 ...
	 *
	 * @param s the stream
	 * @serialData the key (Object) and value (Object) for each key-value mapping, followed by a null pair. The key-value mappings are emitted
	 ...
	 *
	 * @param transformer a function returning the transformation for an element, or null if there is no transformation (in which case the
	 * action is not applied)
	 * @param action the action
	 */
	@SuppressWarnings("unchecked")
	public <U> void forEachSequentially(BiFun<? super K, ? super V, ? extends U> transformer,
					Action<U> action) {
	...
	/**
	 * Returns the result of accumulating the given transformation of all (key, value) pairs using the given reducer to combine values, or
	 * null if none.
	 *
	 * @param transformer a function returning the transformation for an element, or null if there is no transformation (in which case it is
	 * not combined)
	...
	/**
	 * Returns the result of accumulating the given transformation of all (key, value) pairs using the given reducer to combine values, and
	 ...
	 */
	@SuppressWarnings("unchecked")
	public double reduceToDoubleSequentially(ObjectByObjectToDouble<? super K, ? super V> transformer,
					double basis,
	 ...
	 */
	@SuppressWarnings("unchecked")
	public long reduceToLongSequentially(ObjectByObjectToLong<? super K, ? super V> transformer,
					long basis,
	 ...
	 */
	@SuppressWarnings("unchecked")
	public int reduceToIntSequentially(ObjectByObjectToInt<? super K, ? super V> transformer,
					int basis,
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <U> void forEachKeySequentially(Fun<? super K, ? extends U> transformer,
					Action<U> action) {
		if (transformer == null || action == null)
			throw new NullPointerException();
		Traverser<K, V, Object> it = new Traverser<K, V, Object>(this);
	...
	/**
	 * Returns the result of accumulating the given transformation of all keys using the given reducer to combine values, and the given basis
	 ...
	 */
	@SuppressWarnings("unchecked")
	public double reduceKeysToDoubleSequentially(ObjectToDouble<? super K> transformer,
					double basis,
					DoubleByDoubleToDouble reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
	 ...
	 */
	@SuppressWarnings("unchecked")
	public long reduceKeysToLongSequentially(ObjectToLong<? super K> transformer,
					long basis,
					LongByLongToLong reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
	 ...
	 */
	@SuppressWarnings("unchecked")
	public int reduceKeysToIntSequentially(ObjectToInt<? super K> transformer,
					int basis,
					IntByIntToInt reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
	 ...
	 * action is not applied)
	 */
	public <U> void forEachValueSequentially(Fun<? super V, ? extends U> transformer,
					Action<U> action) {
		if (transformer == null || action == null)
			throw new NullPointerException();
		Traverser<K, V, Object> it = new Traverser<K, V, Object>(this);
	...
	/**
	 * Returns the result of accumulating the given transformation of all values using the given reducer to combine values, and the given
	 ...
	 * @return the result of accumulating the given transformation of all values
	 */
	public double reduceValuesToDoubleSequentially(ObjectToDouble<? super V> transformer,
					double basis,
					DoubleByDoubleToDouble reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
	 ...
	 * @return the result of accumulating the given transformation of all values
	 */
	public long reduceValuesToLongSequentially(ObjectToLong<? super V> transformer,
					long basis,
					LongByLongToLong reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
	 ...
	 * @return the result of accumulating the given transformation of all values
	 */
	public int reduceValuesToIntSequentially(ObjectToInt<? super V> transformer,
					int basis,
					IntByIntToInt reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <U> void forEachEntrySequentially(Fun<Map.Entry<K, V>, ? extends U> transformer,
					Action<U> action) {
		if (transformer == null || action == null)
			throw new NullPointerException();
		Traverser<K, V, Object> it = new Traverser<K, V, Object>(this);
	...
	/**
	 * Returns the result of accumulating the given transformation of all entries using the given reducer to combine values, and the given
	 * basis as an identity value.
	 *
	 * @param transformer a function returning the transformation for an element
	 * @param basis the identity (initial default value) for the reduction
	 ...
	 */
	@SuppressWarnings("unchecked")
	public double reduceEntriesToDoubleSequentially(ObjectToDouble<Map.Entry<K, V>> transformer,
					double basis,
					DoubleByDoubleToDouble reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
	 ...
	 */
	@SuppressWarnings("unchecked")
	public long reduceEntriesToLongSequentially(ObjectToLong<Map.Entry<K, V>> transformer,
					long basis,
					LongByLongToLong reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
	 ...
	 */
	@SuppressWarnings("unchecked")
	public int reduceEntriesToIntSequentially(ObjectToInt<Map.Entry<K, V>> transformer,
					int basis,
					IntByIntToInt reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
	 ...
	 * @param action the action
	 */
	public <U> void forEachInParallel(BiFun<? super K, ? super V, ? extends U> transformer,
					Action<U> action) {
		ForkJoinTasks.forEach(this, transformer, action).invoke();
	}

	/**
	 * Returns a non-null result from applying the given search function on each (key, value), or null if none. Upon success, further element
	 * processing is suppressed and the results of any other parallel invocations of the search function are ignored.
	 *
	 * @param searchFunction a function returning a non-null result on success, else null
	 * @return a non-null result from applying the given search function on each (key, value), or null if none
	 */
	 ...
	 * @return the result of accumulating the given transformation of all (key, value) pairs
	 */
	public double reduceToDoubleInParallel(ObjectByObjectToDouble<? super K, ? super V> transformer,
					double basis,
					DoubleByDoubleToDouble reducer) {
	 ...
	 * @return the result of accumulating the given transformation of all (key, value) pairs
	 */
	public long reduceToLongInParallel(ObjectByObjectToLong<? super K, ? super V> transformer,
					long basis,
					LongByLongToLong reducer) {
	 ...
	 * @return the result of accumulating the given transformation of all (key, value) pairs
	 */
	public int reduceToIntInParallel(ObjectByObjectToInt<? super K, ? super V> transformer,
					int basis,
					IntByIntToInt reducer) {
	 ...
	 * @param action the action
	 */
	public <U> void forEachKeyInParallel(Fun<? super K, ? extends U> transformer,
					Action<U> action) {
		ForkJoinTasks.forEachKey(this, transformer, action).invoke();
	}

	/**
	 * Returns a non-null result from applying the given search function on each key, or null if none. Upon success, further element
	 * processing is suppressed and the results of any other parallel invocations of the search function are ignored.
	 *
	 * @param searchFunction a function returning a non-null result on success, else null
	 * @return a non-null result from applying the given search function on each key, or null if none
	 ...
	 * @return the result of accumulating the given transformation of all keys
	 */
	public double reduceKeysToDoubleInParallel(ObjectToDouble<? super K> transformer,
					double basis,
					DoubleByDoubleToDouble reducer) {
	 ...
	 * @return the result of accumulating the given transformation of all keys
	 */
	public long reduceKeysToLongInParallel(ObjectToLong<? super K> transformer,
					long basis,
					LongByLongToLong reducer) {
	 ...
	 * @return the result of accumulating the given transformation of all keys
	 */
	public int reduceKeysToIntInParallel(ObjectToInt<? super K> transformer,
					int basis,
					IntByIntToInt reducer) {
	 ...
	 * action is not applied)
	 */
	public <U> void forEachValueInParallel(Fun<? super V, ? extends U> transformer,
					Action<U> action) {
		ForkJoinTasks.forEachValue(this, transformer, action).invoke();
	}

	/**
	 * Returns a non-null result from applying the given search function on each value, or null if none. Upon success, further element
	 * processing is suppressed and the results of any other parallel invocations of the search function are ignored.
	 *
	 * @param searchFunction a function returning a non-null result on success, else null
	 * @return a non-null result from applying the given search function on each value, or null if none
	 ...
	 * @return the result of accumulating the given transformation of all values
	 */
	public double reduceValuesToDoubleInParallel(ObjectToDouble<? super V> transformer,
					double basis,
					DoubleByDoubleToDouble reducer) {
	 ...
	 * @return the result of accumulating the given transformation of all values
	 */
	public long reduceValuesToLongInParallel(ObjectToLong<? super V> transformer,
					long basis,
					LongByLongToLong reducer) {
	 ...
	 * @return the result of accumulating the given transformation of all values
	 */
	public int reduceValuesToIntInParallel(ObjectToInt<? super V> transformer,
					int basis,
					IntByIntToInt reducer) {
	 ...
	 * @param action the action
	 */
	public <U> void forEachEntryInParallel(Fun<Map.Entry<K, V>, ? extends U> transformer,
					Action<U> action) {
		ForkJoinTasks.forEachEntry(this, transformer, action).invoke();
	}

	/**
	 * Returns a non-null result from applying the given search function on each entry, or null if none. Upon success, further element
	 * processing is suppressed and the results of any other parallel invocations of the search function are ignored.
	 *
	 * @param searchFunction a function returning a non-null result on success, else null
	 * @return a non-null result from applying the given search function on each entry, or null if none
	 ...
	 * @return the result of accumulating the given transformation of all entries
	 */
	public double reduceEntriesToDoubleInParallel(ObjectToDouble<Map.Entry<K, V>> transformer,
					double basis,
					DoubleByDoubleToDouble reducer) {
	 ...
	 * @return the result of accumulating the given transformation of all entries
	 */
	public long reduceEntriesToLongInParallel(ObjectToLong<Map.Entry<K, V>> transformer,
					long basis,
					LongByLongToLong reducer) {
	 ...
	 * @return the result of accumulating the given transformation of all entries
	 */
	public int reduceEntriesToIntInParallel(ObjectToInt<Map.Entry<K, V>> transformer,
					int basis,
					IntByIntToInt reducer) {
	...
	/**
	 * A view of a ConcurrentHashMapV8 as a {@link Set} of keys, in which additions may optionally be enabled by mapping to a common value.
	 * This class cannot be directly instantiated. See 	 {@link #keySet()}, {@link #keySet(Object)}, {@link #newKeySet()},
     * {@link #newKeySet(int)}.
	 */
	public static class KeySetView<K, V> extends CHMView<K, V>
		...
		/**
		 * Returns a "weakly consistent" iterator that will never throw {@link ConcurrentModificationException}, and guarantees to traverse
		 * elements as they existed upon construction of the iterator, and may (but is not guaranteed to) reflect any modifications subsequent
		 * to construction.
		 *
	...
	/**
	 * A view of a ConcurrentHashMapV8 as a {@link Collection} of values, in which additions are disabled. This class cannot be directly
	 * instantiated. See {@link #values()}.
	 *
	 * <p>The view's {@code iterator} is a "weakly consistent" iterator that will never throw {@link ConcurrentModificationException}, and
	 * guarantees to traverse elements as they existed upon construction of the iterator, and may (but is not guaranteed to) reflect any
	 * modifications subsequent to construction.
	...
	/**
	 * A view of a ConcurrentHashMapV8 as a {@link Set} of (key, value) entries. This class cannot be directly instantiated. See
	 * {@link #entrySet()}.
	 */
	...
	// ---------------------------------------------------------------------
	/**
	 * Predefined tasks for performing bulk parallel operations on ConcurrentHashMapV8s. These tasks follow the forms and rules used for bulk
	 * operations. Each method has the same name, but returns a task rather than invoking it. These methods may be useful in custom
	 * applications such as submitting a task without waiting for completion, using a custom pool, or combining with other tasks.
	 */
	public static class ForkJoinTasks {
		...
		/**
		 * Returns a task that when invoked, returns a non-null result from applying the given search function on each (key, value), or null if
		 * none. Upon success, further element processing is suppressed and the results of any other parallel invocations of the search function
		 * are ignored.
		 *
		 * @param map the map
		 * @param searchFunction a function returning a non-null result on success, else null
		...
		public static <K, V, U> ForkJoinTask<U> search(ConcurrentHashMapV8<K, V> map,
			...
			if (searchFunction == null)
				throw new NullPointerException();
			return new SearchMappingsTask<K, V, U>(map, null, -1, searchFunction,
							new AtomicReference<U>());
		}

		/**
		 * Returns a task that when invoked, returns the result of accumulating the given transformation of all (key, value) pairs using the
		...
		/**
		 * Returns a task that when invoked, returns the result of accumulating the given transformation of all (key, value) pairs using the
		 * given reducer to combine values, and the given basis as an identity value.
		 *
		 * @param map the map
		 * @param transformer a function returning the transformation for an element
		 * @param basis the identity (initial default value) for the reduction
		 ...
		 * Upon success, further element processing is suppressed and the results of any other parallel invocations of the search function are
		 * ignored.
		 *
		 * @param map the map
		 * @param searchFunction a function returning a non-null result on success, else null
		...
		public static <K, V, U> ForkJoinTask<U> searchKeys(ConcurrentHashMapV8<K, V> map,
			...
			if (searchFunction == null)
				throw new NullPointerException();
			return new SearchKeysTask<K, V, U>(map, null, -1, searchFunction,
							new AtomicReference<U>());
		}

		/**
		 * Returns a task that when invoked, returns the result of accumulating all keys using the given reducer to combine values, or null if
		 * none.
		 *
		 * @param map the map
		 * @param reducer a commutative associative combining function
		...
		/**
		 * Returns a task that when invoked, returns the result of accumulating the given transformation of all keys using the given reducer to
		...
		public static <K, V, U> ForkJoinTask<U> searchValues(ConcurrentHashMapV8<K, V> map,
			...
			if (searchFunction == null)
				throw new NullPointerException();
			return new SearchValuesTask<K, V, U>(map, null, -1, searchFunction,
							new AtomicReference<U>());
		}

		/**
		 * Returns a task that when invoked, returns the result of accumulating all values using the given reducer to combine values, or null if
		 * none.
		 *
		 * @param map the map
		 * @param reducer a commutative associative combining function
		...
		/**
		 * Returns a task that when invoked, returns the result of accumulating the given transformation of all values using the given reducer
		...
		/**
		 * Returns a task that when invoked, returns the result of accumulating the given transformation of all values using the given reducer
		 * to combine values, and the given basis as an identity value.
		 *
		 * @param map the map
		 * @param transformer a function returning the transformation for an element
		 * @param basis the identity (initial default value) for the reduction
		...
		public static <K, V, U> ForkJoinTask<U> searchEntries(ConcurrentHashMapV8<K, V> map,
			...
			if (searchFunction == null)
				throw new NullPointerException();
			return new SearchEntriesTask<K, V, U>(map, null, -1, searchFunction,
							new AtomicReference<U>());
		}

		/**
		 * Returns a task that when invoked, returns the result of accumulating all entries using the given reducer to combine values, or null
		 * if none.
		 *
		 * @param map the map
		 * @param reducer a commutative associative combining function
		...
		/**
		 * Returns a task that when invoked, returns the result of accumulating the given transformation of all entries using the given reducer
		 * to combine values, or null if none.
		 *
		 * @param map the map
		 * @param transformer a function returning the transformation for an element, or null if there is no transformation (in which case it is
	...
	static final class ForEachKeyTask<K, V>
		...
		ForEachKeyTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
						Action<K> action) {
			super(m, p, b);
			this.action = action;
		}
	...
	static final class ForEachValueTask<K, V>
		...
		ForEachValueTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
						Action<V> action) {
			super(m, p, b);
			this.action = action;
		}
	...
	static final class ForEachEntryTask<K, V>
		...
		ForEachEntryTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
	...
	static final class ForEachMappingTask<K, V>
		...
		ForEachMappingTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
						BiAction<K, V> action) {
			super(m, p, b);
			this.action = action;
		}
	...
	static final class ReduceKeysTask<K, V>
		...
		ReduceKeysTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
	...
	static final class ReduceValuesTask<K, V>
		...
		ReduceValuesTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
	...
	static final class ReduceEntriesTask<K, V>
		...
		ReduceEntriesTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
	...
	static final class MapReduceKeysToDoubleTask<K, V>
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null
				...
				double r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceKeysToDoubleTask<K, V>(map, this, b, rights, transformer, r, reducer)).fork();
				while (advance() != null)
	...
	static final class MapReduceValuesToDoubleTask<K, V>
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null
				...
				double r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceValuesToDoubleTask<K, V>(map, this, b, rights, transformer, r, reducer)).fork();
				V v;
				while ((v = advance()) != null)
					r = reducer.apply(r, transformer.apply(v));
				result = r;
	...
	static final class MapReduceEntriesToDoubleTask<K, V>
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null
				...
				double r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceEntriesToDoubleTask<K, V>(map, this, b, rights, transformer, r, reducer)).fork();
				V v;
				while ((v = advance()) != null)
	...
	static final class MapReduceMappingsToDoubleTask<K, V>
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null
				...
				double r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceMappingsToDoubleTask<K, V>(map, this, b, rights, transformer, r, reducer)).fork();
				V v;
				while ((v = advance()) != null)
	...
	static final class MapReduceValuesToLongTask<K, V>
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null
				...
				long r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceValuesToLongTask<K, V>(map, this, b, rights, transformer, r, reducer)).fork();
				V v;
				while ((v = advance()) != null)
					r = reducer.apply(r, transformer.apply(v));
				result = r;
	...
	static final class MapReduceEntriesToLongTask<K, V>
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null
				...
				long r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceEntriesToLongTask<K, V>(map, this, b, rights, transformer, r, reducer)).fork();
				V v;
				while ((v = advance()) != null)
					r = reducer.apply(r, transformer.apply(entryFor((K) nextKey,
									v)));
	...
	static final class MapReduceMappingsToLongTask<K, V>
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null
				...
				long r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceMappingsToLongTask<K, V>(map, this, b, rights, transformer, r, reducer)).fork();
				V v;
				while ((v = advance()) != null)
					r = reducer.apply(r, transformer.apply((K) nextKey, v));
				result = r;
	...
	static final class MapReduceEntriesToIntTask<K, V>
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null
				...
				int r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceEntriesToIntTask<K, V>(map, this, b, rights, transformer, r, reducer)).fork();
				V v;
				while ((v = advance()) != null)
					r = reducer.apply(r, transformer.apply(entryFor((K) nextKey,
									v)));
	...
	static final class MapReduceMappingsToIntTask<K, V>
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null
				...
				int r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceMappingsToIntTask<K, V>(map, this, b, rights, transformer, r, reducer)).fork();
				V v;
				while ((v = advance()) != null)
					r = reducer.apply(r, transformer.apply((K) nextKey, v));
				result = r;
	...
	/**
	 * Returns a sun.misc.Unsafe. Suitable for use in a 3rd party package. Replace with a simple call to Unsafe.getUnsafe when integrating
	 * into a jdk.
	 *
