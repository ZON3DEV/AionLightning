...
public class AttackUtil {
	...
	private static float calculateWeaponCritical(Creature attacked, float damages, WeaponType weaponType, int critAddDmg, StatEnum stat) {
		...
		if (weaponType != null) {
			...
			if (stat.equals(StatEnum.MAGICAL_CRITICAL_DAMAGE_REDUCE)) {
				coeficient = 1.5f; // Magical skill with physical weapon TODO:
									// confirm this
			}
		}

		if (attacked instanceof Player) { // Strike Fortitude lowers the crit
											// multiplier
	 ...
	 * @param accMod
	 */
	public static void calculateSkillResult(Effect effect, int skillDamage, ActionModifier modifier, Func func, int randomDamage, int accMod, int criticalProb,
			int critAddDmg, boolean cannotMiss, boolean shared, boolean ignoreShield) {
		...
		if (randomDamage > 0) {
			...
			switch (randomDamage) {
				...
				default:
					/*
					 * chance to do from 50% to 200% damage This must NOT be
					 * calculated after critical status check, or it will be
	...
	private static final AttackStatus calculateMagicalMainHandResult(Creature attacker, Creature attacked, AttackStatus attackerStatus, int damage,
			List<AttackResult> attackList) {
		AttackStatus mainHandStatus = attackerStatus;
		if (mainHandStatus == null)
	...
	private static final void calculateMagicalOffHandResult(Creature attacker, Creature attacked, AttackStatus attackerStatus, List<AttackResult> attackList,
			SkillElement elem) {
	...
	private static final List<AttackResult> splitMagicalDamage(final Creature attacker, final Creature attacked, int damage, AttackStatus status,
			List<AttackResult> attackList) {
		...
		switch (status) {
			...
			case CRITICAL:
				if (attacker instanceof Player)
					damage = (int) calculateWeaponCritical(attacked, damage, ((Player) attacker).getEquipment().getMainHandWeaponType(),
	...
	public static int calculateMagicalOverTimeSkillResult(Effect effect, int skillDamage, SkillElement element, int position, boolean useMagicBoost,
		...
		int damage = Math.round(StatFunctions.calculateMagicalSkillDamage(effect.getEffector(), effect.getEffected(), skillDamage, 0, element, useMagicBoost,
	...
	public static void calculateMagicalSkillResult(Effect effect, int skillDamage, ActionModifier modifier, SkillElement element, boolean useMagicBoost,
		...
		float damageMultiplier = effector.getObserveController().getBaseMagicalDamageMultiplier();
		int baseAttack = effector.getGameStats().getMainHandPAttack().getBase(); // Npc
																					// spells
																					// scale
																					// with
																					// this
		...
		int damage = Math.round(StatFunctions.calculateMagicalSkillDamage(effect.getEffector(), effect.getEffected(), damages, bonus, element, useMagicBoost,
				useKnowledge, noReduce, effect.getSkillTemplate().getPvpDamage()) * damageMultiplier);
	 ...
	 *
	 * @source
	 *         http://www.aionsource.com/forum/mechanic-analysis/42597-character-
	 *         stats-xp-dp-origin-gerbator-team-july-2009 -a.html
	 * @return AttackStatus
	 */
	...
	public static AttackStatus calculatePhysicalStatus(Creature attacker, Creature attacked, boolean isMainHand, int accMod, int criticalProb, boolean isSkill,
			boolean cannotMiss) {
		AttackStatus status = AttackStatus.NORMALHIT;
		if (!isMainHand)
		...
		if (!cannotMiss) {
			if (attacked instanceof Player && ((Player) attacked).getEquipment().isShieldEquipped()
					&& StatFunctions.calculatePhysicalBlockRate(attacker, attacked))// TODO
																					// accMod
				status = AttackStatus.BLOCK;
			// Parry can only be done with weapon, also weapon can have humanoid
			// mobs,
			// but for now there isnt implementation of monster category
					...
					&& StatFunctions.calculatePhysicalParryRate(attacker, attacked))// TODO
																					// accMod
