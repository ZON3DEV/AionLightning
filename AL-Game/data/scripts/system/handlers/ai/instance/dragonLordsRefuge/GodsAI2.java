		...
		if (getNpcId() == 219488 || getNpcId() == 219491) {
			...
			ThreadPoolManager.getInstance().schedule(new Runnable() {
				...
				public void run() {
					...
					SkillEngine.getInstance().getSkill(getOwner(), (getNpcId() == 219488 ? 20931 : 20935), 60, tiamat).useNoAnimationSkill(); // adds
																																				// 1mio
																																				// hate
				}
			}, 12000);
		} else if (getNpcId() == 219492 || getNpcId() == 219489) {
			// empyrean lord (god) start final attack to Tiamat Dragon before
			// became exausted
