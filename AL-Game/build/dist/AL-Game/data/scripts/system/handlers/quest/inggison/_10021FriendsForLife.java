							...
							if (var == 4) {
								if (player.isInGroup2()) {
									...
									if (giveQuestItem(env, 182206627, 1) && giveQuestItem(env, 182206628, 1)) {
										...
										TeleportService2
												.teleportTo(player, 300190000, newInstance.getInstanceId(), 202.26694f, 226.0532f, 1098.236f, (byte) 30);
	...
	public boolean onKillEvent(QuestEnv env) {
		...
		if (qs.getStatus() == QuestStatus.START) {
			...
			if (var == 3) {
				...
				if (var1 >= 0 && var1 < 9) {
					return defaultOnKillEvent(env, mobs, var1, var1 + 1, 1); // 1:
																				// 1
																				// -
																				// 9
				} else if (var1 == 9) {
			...
			if (player.getWorldId() == 300190000) {
				...
				if (itemId == 182206627) {
					if (var == 5) {
						...
						return HandlerResult.SUCCESS; // TODO: Should not
														// remove, but use the
														// skill
					}
					...
					if (var == 6) {
						...
						if (var2 >= 0 && var2 < 19) {
							changeQuestStep(env, var2, var2 + 1, false, 2); // 2:
																			// 1
																			// -
																			// 19
