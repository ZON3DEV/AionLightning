...
public class CrucibleChallengeInstance extends CrucibleInstance {
					...
					public void run() {
						...
						if (player != null) {
							...
							if (qs != null && qs.getStatus() == QuestStatus.START) {
								if (qs.getQuestVarById(0) == 1 || qs.getQuestVarById(1) == 4) { // 5
																								// x
																								// Vanktrist
																								// Spacetwine
																								// killed
	...
	public boolean onDie(final Player player, Creature lastAttacker) {
		...
		int place = 0;
		if (isInZone(ZoneName.get("ILLUSION_STADIUM_11_300320000"), player)) { // stage
																				// 1
			place = 1;
		} else if (isInZone(ZoneName.get("ILLUSION_STADIUM_15_300320000"), player)) { // stage
																						// 2
			place = 2;
		} else if (isInZone(ZoneName.get("ILLUSION_STADIUM_12_300320000"), player)) { // stage
																						// 3
			place = 3;
		} else if (isInZone(ZoneName.get("ILLUSION_STADIUM_8_300320000"), player)) { // stage
																						// 4
			place = 4;
		} else if (isInZone(ZoneName.get("ILLUSION_STADIUM_13_300320000"), player)) { // stage
																						// 4
			place = 5;
		} else if (isInZone(ZoneName.get("ILLUSION_STADIUM_9_300320000"), player)) { // stage
																						// 5
			place = 6;
		} else if (isInZone(ZoneName.get("ILLUSION_STADIUM_14_300320000"), player)) { // stage
																						// 6
			place = 7;
		}
	...
	public void onChangeStage(StageType type) {
		...
		switch (stageType) {
			...
			case START_ALTERNATIVE_STAGE_4_ROUND_1:
				switch (Rnd.get(1, 3)) {
					case 1:
						npcId = 217791; // http://www.aiondatabase.com/npc/217805
										// spawn 2 5%
						break;
					case 2:
						npcId = 217792;
