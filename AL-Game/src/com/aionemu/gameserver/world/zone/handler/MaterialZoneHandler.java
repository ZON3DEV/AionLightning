...
public class MaterialZoneHandler implements ZoneHandler {
	...
	public MaterialZoneHandler(Spatial geometry, MaterialTemplate template) {
		...
		if (name.contains("FIRE_BOX") || name.contains("FIRE_SEMISPHERE") || name.contains("FIREPOT") || name.contains("FIRE_CYLINDER")
				|| name.contains("FIRE_CONE") || name.startsWith("BU_H_CENTERHALL")) {
		}
	...
	public void onEnterZone(Creature creature, ZoneInstance zone) {
		...
		/*
		 * CollisionMaterialActor actor = new CollisionMaterialActor(creature,
		 * geometry, template);
		 * creature.getObserveController().addObserver(actor);
		 * observed.put(creature.getObjectId(), actor); if (actOnEnter)
		 * actor.act();
		 */
	}
