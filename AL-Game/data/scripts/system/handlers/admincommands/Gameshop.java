...
public class Gameshop extends AdminCommand {
	...
	public void execute(Player admin, String... params) {
		...
		if ("delete".startsWith(params[0])) {
			...
			DAOManager.getDAO(InGameShopDAO.class).saveIngameShopItem(IDFactory.getInstance().nextId(), itemId, count, price, category, subCategory, list - 1,
			...
			if (params.length == 2) {
				...
				if (target instanceof Player) {
					...
					PacketSendUtility.sendMessage(admin, "You should select a player");
				}
			}
