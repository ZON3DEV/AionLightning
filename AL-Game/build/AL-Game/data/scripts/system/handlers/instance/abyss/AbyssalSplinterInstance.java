...
public class AbyssalSplinterInstance extends GeneralInstanceHandler {
		...
		switch (npcId) {
			...
			case 216949: // ebonsoul
				if (getNpc(npcId == 216949 ? 216948 : 216949) == null) {
					...
					sendMsg(npcId == 216948 ? 1400634 : 1400635); // Defeat
																	// Rukril/Ebonsoul
																	// in 1 min!
	...
	public void handleUseItemFinish(Player player, Npc npc) {
		if (npc.getNpcId() == 700856) {
			...
			if (!isSpawned(216960) && !isSpawned(216952) && !bossSpawned) { // No
																			// bosses
																			// spawned
				if (!isSpawned(700955) && destroyedFragments == 3) { // No Huge
																		// Aether
																		// Fragments
																		// spawned
																		// (all
	...
	private void onFragmentKill() {
		switch (destroyedFragments) {
			case 1:
				// The destruction of the Huge Aether Fragment has destabilized
				// the artifact!
				sendMsg(1400689);
				break;
			case 2:
				// The destruction of the Huge Aether Fragment has put the
				// artifact protector on alert!
				sendMsg(1400690);
				break;
			case 3:
				// The destruction of the Huge Aether Fragment has caused
				// abnormality on the artifact. The artifact protector is
				// furious!
				sendMsg(1400691);
				break;
		}
