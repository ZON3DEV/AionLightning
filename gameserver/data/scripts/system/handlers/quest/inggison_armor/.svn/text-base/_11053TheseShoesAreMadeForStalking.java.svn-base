...
public class _11053TheseShoesAreMadeForStalking extends QuestHandler
{
       ...
       if(qs == null || qs.getStatus() == QuestStatus.NONE) 
       {
        ...
        if (qs.getStatus() == QuestStatus.START)
		{
            switch (targetId)
			{
	          ...
	          {
                                player.getInventory().removeFromBagByItemId(182206838, 30);
    ...
    public void register()
	{
        ...
        qe.setNpcQuestData(799015).addOnQuestStart(questId); //Siaqua
        qe.setNpcQuestData(799015).addOnTalkEvent(questId); //Siaqua
    }
}